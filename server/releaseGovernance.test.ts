...
describe("black-box release contract", () => {
  it("exposes the approved product release and audited runtime", () => {
    expect(APP_VERSION).toBe("2.0.204");
    expect(RELEASE_LABEL).toBe("JARVI RH 2.0.204");
  ...
  it("enforces formal institutional treatment across runtime surfaces", () => {
    ...
    // 2.0.204: +1 por la costura de almacenamiento del RAG (server/storageBackend.ts).
    // 2.0.204: +1 por la conexión de Google Drive (server/driveConnection.ts).
    // 2.0.204: +1 por el backend de almacenamiento en Drive (server/driveStorage.ts).
    expect(audit.files).toHaveLength(153);
    expect(audit.findings).toEqual([]);
    expect(publicCopyAudit.files).toHaveLength(153);
  ...
  it("publishes the academic-commercial README with auditable proportions and references", () => {
    ...
    expect(readme).toContain("Talento AISA · JARVI RH 2.0.204");
    ...
    //
    // El techo del registro se elevó de 2600 a 3500: la entrega acumulada de
    ...
    expect(historyWordCount).toBeLessThanOrEqual(3_500);
    expect(bibliography).toHaveLength(41);
    expect(readme).toContain("### API, infraestructura y modelos");
    expect(readme).toContain("<!-- release-history:start -->");
    expect(readme).toContain("### 22SEP2026 · JARVI RH 2.0.204");
  ...
  it("declara el despliegue separado por capacidad con cola dedicada", () => {
    ...
    expect(governance).toContain("Alcance candidato 2.0.204");
  ...
  it("conserva la integridad del relato de release en cada entrega", () => {
    ...
    // entrega de 2.0.166 llegó a citar 2.0.204. La hoja de especificación del
