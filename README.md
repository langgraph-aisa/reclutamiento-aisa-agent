...
<h1 align="justify">Empleos de Energia Solar en Guatemala | Talento AISA · JARVI RH 2.0.205</h1><p></p>
...
<p></p>
<p align="left">
  <img src="https://img.shields.io/badge/JARVI%20RH-2.0.205-0b2d4b" alt="JARVI RH 2.0.205" />
...
<p align="center">
  ...
  <br />
  <sub>Agente JARVI RH 2.0.205 de Talento AISA (IA Evaluadora).</sub>
</p>
...
- La costura pasa a clave relativa y el backend de Drive resuelve cada clave contra `JARVI RH/<proyecto>/…`, con renovación del token y la misma semántica que el local. Sin migración.

### 22SEP2026 · JARVI RH 2.0.202

- La costura `server/storageBackend.ts` separa escribir, leer, borrar y medir de su implementación: los métodos vigentes despachan al backend y un backend de Drive podrá sustituir al volumen local sin tocar los puntos de llamada.
- La conexión de Google Drive queda disponible: credencial de plataforma en Configuración y autorización personal en «Mi cuenta» con scope `drive.file`, `refresh_token` cifrado y revocación. Sin migración.
...
- La puerta de integración continua deja de fallar por azar: la aplicación de migraciones se serializa entre archivos de prueba. La carrera era real —la migración consulta `pg_roles` y después crea el rol, de modo que dos migradores simultáneos ven «no existe» los dos— y el rol es del clúster, así que dos bases de prueba distintas colisionaban entre sí.
- La defensa vive en el arnés y no en cada guarda de cada migración: corregir una guarda no cierra la clase, y cualquier migración futura podría reintroducirla. `CREATE SCHEMA IF NOT EXISTS` tiene la misma forma.
- La carrera se verifica de forma deliberada con `pnpm verify:migrations`, que mide las dos mitades del hecho: que existe sin cerrojo —se reprodujo en seis de diez rondas— y que el cerrojo la cierra.
