...
agent_nodes = [
    ...
    node("Evaluar respuestas abiertas", "@n8n/n8n-nodes-langchain.chainLlm", 1.7, [1020, 0], {"promptType": "define", "text": "=Evalúe a la persona candidata para la plaza {{ $json.application?.job_position_id }}. Respete las reglas deterministas y razone las respuestas abiertas según evaluation_criteria y ai_prompt. Entregue un puntaje de 0 a 100 y asigne el estado exactamente así: 90–100 pre_calificado_prioritario; 80–89 pre_calificado; 70–79 pre_calificado_condicionado; 60–69 pendiente_revision_humana; 0–59 no_calificado. Si deterministic.passed es false, el estado final debe ser no_calificado sin importar el score. Devuelva un JSON con score, status, reason, profileSummary, keyPoints y confidence.\n\nDatos: {{ JSON.stringify($json) }}", "hasOutputParser": True}, None),
...
whatsapp_nodes = [
    ...
    node("Preparar mensajes", "n8n-nodes-base.code", 2, [260, 0], {"jsCode": "const input = $json.body ?? $json;\nconst position = input.positionTitle ?? 'la plaza';\nconst recipientList = input.internalRecipients ?? [];\nreturn [{ json: { ...input, candidateMessage: `Gracias por postularse a la plaza de ${position}. Agradeceremos que nos envíe su currículum vitae para continuar con el proceso de evaluación.`, internalMessages: recipientList.map(phone => ({ phone, message: `Nuevo candidato calificado para entrevista: ${position}. Teléfono: ${input.phoneInternational}` })) } }];"}),
