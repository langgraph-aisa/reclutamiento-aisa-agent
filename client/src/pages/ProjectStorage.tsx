...
export default function ProjectStorage() {
  const projects = trpc.drive.projects.useQuery();
  const connectedUsers = trpc.drive.connectedUsers.useQuery();

  const setMode = trpc.drive.setProjectStorage.useMutation({
    onSuccess: async () => {
  ...
  const assign = trpc.drive.assignProjectConnection.useMutation({
    onSuccess: async () => {
      await projects.refetch();
      toast.success("Cuenta de Drive del proyecto actualizada.");
    },
    onError: error => toast.error(error.message),
  });

  const migrate = trpc.drive.migrateProject.useMutation({
    ...
    <div className="max-w-5xl space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-muted-foreground">
          ...
          volumen local o en el Google Drive de la cuenta que lo respalda.
          ...
          <Card key={project.id}>
            <CardHeader>
              <div className="flex items-start justify-between gap-4">
                ...
                >
                  {project.storage_mode === "drive" ? (
                    <>
                      <Cloud className="mr-1 h-3.5 w-3.5" /> Drive
                    </>
                  ) : (
                    <>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {project.storage_mode === "drive" ? (
                  ...
                  <Button
                    disabled={setMode.isPending}
                    onClick={() =>
                      setMode.mutate({ projectId: project.id, mode: "drive" })
                    }
                  >
              ...
              <div className="space-y-2">
                <p className="text-sm font-semibold uppercase tracking-[0.18em] text-primary">
                  Cuenta de Drive asignada
                </p>
                <Select
                  value={
                    project.drive_connection_user_id != null
                      ? String(project.drive_connection_user_id)
                      : "owner"
                  }
                  onValueChange={value =>
                    assign.mutate({
                ...
                <p className="text-xs text-muted-foreground">
                  Solo las cuentas que ya conectaron su Drive aparecen en la
