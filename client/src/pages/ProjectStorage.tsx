...
import { DropboxTreeBrowser, type DropboxTreeEntry } from "@/components/DropboxTreeBrowser";
 ...
 *
 * La navegación y la previsualización viven en la pieza compartida con el RAG
 * del candidato: esta hoja aporta su origen de datos —el árbol del proyecto— y
 * su acuñador de vales, de modo que las dos superficies no divergan al
...
function DropboxBrowser({ projectId }: { projectId: number }) {
  ...
  return (
    <DropboxTreeBrowser
      title="RAG del proyecto · árbol de Dropbox"
      root={tree.data?.root ?? null}
      relativePath={path}
      entries={(tree.data?.entries ?? []) as DropboxTreeEntry[]}
      available={Boolean(tree.data?.available)}
      loading={tree.isLoading}
      unavailableLabel="El proyecto no custodia sus documentos en Dropbox."
      onNavigate={setPath}
