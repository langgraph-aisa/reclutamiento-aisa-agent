      ...
      <Tabs defaultValue="whatsapp" className="space-y-5">
        ...
        <TabsContent value="whatsapp" className="space-y-5">
          ...
          <Card className="rounded-3xl border-0 shadow-soft">
            ...
            <CardContent className="space-y-4"><div className="grid gap-3 sm:grid-cols-[1fr_1fr_auto]"><Input value={recipient.label} onChange={e => setRecipient({ ...recipient, label: e.target.value })} placeholder="Nombre o equipo" className="rounded-2xl" /><GuatemalaPhoneInput value={recipient.phone} onChange={phone => setRecipient({ ...recipient, phone })} className="rounded-2xl" ariaLabel="Teléfono interno de Guatemala" /><Button onClick={addRecipient} disabled={saveRecipient.isPending} className="rounded-2xl"><Plus className="mr-2 h-4 w-4" /> Agregar</Button></div><div className="divide-y divide-border/70 rounded-2xl border border-border/70">{recipients.data?.length ? recipients.data.map((row: any) => <div key={row.id} className="flex items-center justify-between p-4"><div><p className="font-semibold text-primary">{row.label}</p><p className="text-sm text-muted-foreground">{row.phone_international}</p></div><Badge className="rounded-full bg-emerald-100 text-emerald-800 hover:bg-emerald-100">Activo</Badge></div>) : <p className="p-5 text-sm text-muted-foreground">Agregue el primer número interno.</p>}</div></CardContent>
          </Card>
        </TabsContent>
