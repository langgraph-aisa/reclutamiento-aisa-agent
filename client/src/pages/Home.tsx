...
export default function Home() {
    ...
    return (
      <main className="min-h-screen overflow-hidden grid-paper">
        <div className="mx-auto max-w-7xl px-4 py-5 sm:px-7 sm:py-7 lg:px-8">
          ...
          <section className="grid gap-4 border-t border-primary/10 py-7 sm:grid-cols-3"><Feature icon={SunMedium} title="Construya su carrera transformando energía en oportunidades." /><Feature icon={GraduationCap} title="Crezca profesionalmente con tecnología, propósito y aprendizaje." /><Feature icon={Lightbulb} title="Convierta sus habilidades en soluciones que transforman." /></section>
  ...
  const metrics = [{ label: "Postulaciones", value: stats.total, note: "Todas las plazas", icon: UsersRound, tone: "bg-sky-50 text-sky-700" }, { label: "En revisión", value: stats.enRevision, note: "Requieren atención", icon: ClipboardCheck, tone: "bg-amber-50 text-amber-700" }, { label: "Solicitar CV por WhatsApp", value: stats.calificados, note: "Acciones registradas", icon: Sparkles, tone: "bg-emerald-50 text-emerald-700" }, { label: "Calificado por AISA", value: stats.calificadosAisa, note: "Sin evento automático", icon: ClipboardCheck, tone: "bg-violet-50 text-violet-700" }, { label: "Entrevistas", value: stats.entrevistas, note: "En cualquier estado", icon: MessageCircle, tone: "bg-fuchsia-50 text-fuchsia-700" }];
  return <div className="space-y-8"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm font-semibold uppercase tracking-[.18em] text-emerald-700">Centro de mando</p><h1 className="mt-2 text-4xl font-800 tracking-[-.04em] text-primary">Buenos días, {user.name?.split(" ")[0] ?? "equipo"}.</h1><p className="mt-2 text-muted-foreground">Una vista rápida de la operación de selección.</p></div><Link href="/admin/jobs"><Button className="rounded-full">Gestionar plazas <ArrowRight className="ml-2 h-4 w-4" /></Button></Link></div><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">{metrics.map(metric => <Card key={metric.label} className="rounded-3xl border-0 shadow-soft"><CardContent className="p-5"><div className="flex items-start justify-between"><div><p className="text-sm text-muted-foreground">{metric.label}</p><p className="mt-3 text-4xl font-800 tracking-tight text-primary">{metric.value}</p><p className="mt-2 text-xs text-muted-foreground">{metric.note}</p></div><div className={`grid h-11 w-11 place-items-center rounded-2xl ${metric.tone}`}><metric.icon className="h-5 w-5" /></div></div></CardContent></Card>)}</div><div className="grid gap-5 lg:grid-cols-[1.35fr_.65fr]"><Card className="rounded-3xl border-0 shadow-soft"><CardHeader className="flex flex-row items-center justify-between"><div><CardTitle className="text-xl text-primary">Su operación</CardTitle><p className="mt-1 text-sm text-muted-foreground">Atajos para las tareas más frecuentes.</p></div><Badge variant="outline" className="rounded-full">{stats.positions} plazas</Badge></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2"><Quick href="/admin/jobs" icon={LayoutDashboard} title="Configurar plazas" text="Publique enlaces seguros y asigne un agente." /><Quick href="/admin/candidates" icon={UsersRound} title="Revisar candidatos" text="Filtre estados y registre decisiones humanas." /><Quick href="/admin/reports" icon={Database} title="Ver informes" text="Convierta el flujo en señales accionables." /><Quick href="/admin/config" icon={MessageCircle} title="Integraciones" text="Administre receptores y variables de conexión." /></CardContent></Card><Card className="rounded-3xl border-0 bg-[#0b2d4b] text-white shadow-soft dark:bg-[#162333]"><CardHeader><CardTitle className="text-xl text-white">Principio operativo</CardTitle></CardHeader><CardContent><p className="text-3xl font-800 leading-tight">Reglas claras. Contexto completo. Siguiente paso oportuno.</p><p className="mt-5 text-sm leading-6 text-white/65">La acción “Solicitar CV por WhatsApp” conserva una ventana de 30 segundos antes de activar la continuación.</p></CardContent></Card></div></div>;
}
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="rounded-2xl bg-white/8 p-4">
              ...
              <p className="mt-2 text-sm leading-6 text-white/70">{selectedJob.description || "Conozca esta oportunidad y complete el formulario para participar en el proceso de selección."}</p>
            </div>
            ...
            <div className="rounded-2xl border border-emerald-300/20 bg-emerald-300/10 p-3.5">
              ...
              <p className="mt-2 text-sm leading-6 text-white/80">{selectedJob.profileSummary || selectedJob.description || "Consulte los requisitos completos al iniciar el formulario."}</p>
            </div>

            <Link href={`/apply/${selectedJob.token}`}>
              <Button size="lg" className="w-full rounded-2xl bg-white text-[#0B2945] hover:bg-white/90 dark:bg-primary dark:text-primary-foreground dark:hover:bg-primary/90">
