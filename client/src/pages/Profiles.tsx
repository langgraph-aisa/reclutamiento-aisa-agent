...
const toForm = (profile: any) => ({
  ...
  experienceYearsMin: profile.experience_years_min?.toString() ?? "",
  experienceYearsMax: profile.experience_years_max?.toString() ?? "",
  ...
  aiCriteria: profile.ai_criteria ?? "",
...
export default function Profiles() {
  ...
  const remove = trpc.profiles.remove.useMutation({
    onSuccess: () => {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    save.mutate({
      ...
      experienceYearsMin: form.experienceYearsMin
        ...
        : undefined,
      experienceYearsMax: form.experienceYearsMax
      ...
      aiCriteria: form.aiCriteria || undefined,
      positionIds: split(form.positionIds).map(Number).filter(Number.isInteger),
    });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.18em] text-primary">
            Configuración de talento
          </p>
          <h1 className="mt-2 text-3xl font-semibold tracking-tight">
            Perfiles laborales
          </h1>
          <p className="mt-2 text-muted-foreground">
            Defina el perfil requerido y los criterios que alimentarán futuros
            formularios y evaluaciones IA.
          </p>
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1.35fr]">
        <Card>
          <CardHeader>
            ...
            <div className="relative">
              ...
              <Input
                className="pl-9"
                placeholder="Buscar por nombre, resumen o nivel"
                value={search}
              ...
              <select
                ...
                className="mt-3 flex h-10 w-full rounded-xl border border-input bg-background px-3 text-sm"
              >
            ...
            {profiles.isLoading ? (
              ...
              profiles.data.map(profile => (
                <button
                  key={profile.id}
                  type="button"
                  className="w-full rounded-xl border border-border/70 p-4 text-left transition hover:border-primary/50 hover:bg-accent/30"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-sm text-muted-foreground line-clamp-2">
                        {profile.summary || "Sin resumen definido"}
                      </p>
                    </div>
                  </div>
                  <div className="mt-3 flex flex-wrap gap-2 text-xs text-muted-foreground">
                    <span>{profile.academic_level || "Nivel no definido"}</span>
                    <span>·</span>
                    <span>{profile.experience_years_min ?? 0}+ años</span>
                    <span>·</span>
                    <span
                      className={
                        profile.active ? "text-emerald-700" : "text-amber-700"
                      }
                    >
              ...
              <p className="rounded-xl bg-muted/40 p-4 text-sm text-muted-foreground">
                Aún no hay perfiles. Cree el primero para comenzar.
              </p>
            )}
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">Definición del perfil</CardTitle>
          </CardHeader>
          <CardContent>
            <form className="space-y-5" onSubmit={submit}>
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2 sm:col-span-2">
                  ...
                  <Input
                    ...
                    placeholder="Ej. Vendedor de campo"
                    required
                  />
                </div>
                <div className="space-y-2 sm:col-span-2">
                ...
                <div className="space-y-2 sm:col-span-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Qué resultado debe lograr la persona"
                  />
                </div>
                <Field
                ...
                <Field
                  ...
                  value={form.requiredRequirements}
                  onChange={v => update("requiredRequirements", v)}
                />
                <Field
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={event =>
                      update("academicLevel", event.target.value)
                    }
                    placeholder="Diversificado, técnico, universitario…"
                  />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={event =>
                        update("experienceYearsMin", event.target.value)
                      }
                    />
                  </div>
                  <div className="space-y-2">
                    <Label>Experiencia máxima</Label>
                    <Input
                      type="number"
                      min="0"
                      value={form.experienceYearsMax}
                      onChange={event =>
                        update("experienceYearsMax", event.target.value)
                      }
                    />
                  </div>
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Inmediata, turnos, fines de semana…"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Guatemala · municipios o zona"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Presencial, híbrida o remota"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={event =>
                      update("salaryRange", event.target.value)
                    }
                    placeholder="Q…"
                  />
                </div>
                <div className="space-y-2 sm:col-span-2">
                  <Label>Criterios para IA</Label>
                  <Textarea
                    value={form.aiCriteria}
                    onChange={event => update("aiCriteria", event.target.value)}
                    placeholder="Cómo debe razonar experiencia, incongruencias y evidencia del candidato"
                  />
                </div>
                <div className="space-y-2 sm:col-span-2">
                  <Label>IDs de plazas asociadas</Label>
                  <Input
                    value={form.positionIds}
                    onChange={event =>
                      update("positionIds", event.target.value)
                    }
              ...
              <div className="flex flex-wrap gap-3">
                ...
                {form.id && (
                  <Button
                    type="button"
                    variant={form.active === false ? "default" : "destructive"}
                    disabled={remove.isPending || setActive.isPending}
                    onClick={() =>
                      form.active === false
                        ? setActive.mutate({ id: form.id!, active: true })
