...
ApiChat publica, según su OpenAPI nativo, un objeto `messages` cuyos elementos incluyen `id`, `number`, `type`, `from_me` y, para texto, `text`. `04_whatsapp_apichat.json` expone `POST /webhook/apichat/incoming`, ignora mensajes salientes, grupos y tipos no textuales, convierte el teléfono al formato E.164 y conserva el identificador original. Luego entrega el contrato anterior al backend mediante una credencial Header Auth llamada «Cabecera interna Talento AISA».
