...
**El defecto.** La hoja de Configuración inicializaba la caja con un texto quemado en el cliente y nunca leía `whatsapp_message`: guardar escribía en la base pero la interfaz no lo reflejaba, y al recargar reaparecía el texto por defecto.

**La corrección.** `client/src/pages/Config.tsx` consume `config.settings` y carga el mensaje base y el país predeterminado desde `integration_settings`, con bandera para no pisar la edición en curso; el guardado declara su resultado con aviso.
