# Validación final · JARVI RH 2.0.136

Fecha de ejecución: 2026-09-14. Rama objetivo: `main`.

## Alcance verificado

Esta hoja registra la corrección del guardado de análisis: «Guardar análisis» declara `::varchar` explícito en el UPDATE de `knowledge_files` para alinear la deducción de tipos de PostgreSQL y eliminar el error «inconsistent types deduced for parameter $1»; la Descripción del Proyecto acomoda el Resumen bajo el Nombre, antes de las plazas vinculadas. La vinculación múltiple de plazas al RAG de 2.0.135 (migración `0017`), Administrador de Proyectos, bandeja ApiChat directa, observabilidad Langfuse y las capacidades de 2.0.134 permanecen bajo regresión. Los casos observables están en [PRUEBAS_CAJA_NEGRA_2.0.136.md](PRUEBAS_CAJA_NEGRA_2.0.136.md) y el protocolo operacional en [OBSERVABILIDAD_LANGFUSE_2.0.131.md](OBSERVABILIDAD_LANGFUSE_2.0.131.md).

## Evidencia automatizada

| Validación                                         | Resultado                     |
| -------------------------------------------------- | ----------------------------- |
| Gobierno (`pnpm release:verify`)                   | Aprobado · `JARVI RH 2.0.136` |
| Tratamiento y cobertura (`pnpm text:verify`)       | Aprobado · 89 archivos        |
| Próxima versión (`pnpm release:bump -- --dry-run`) | Aprobado · indica `2.0.136 → 2.0.137` |
